...
def test_help_command_includes_custom_commands() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("clippy.cli.custom_commands.get_user_manager") as mock_user_mgr:
            ...
            help_content = console.print.call_args[0][0]
            # The Rich Panel doesn't show the content in str(), so check that it was called
            # with a Panel (which indicates help was formatted)
