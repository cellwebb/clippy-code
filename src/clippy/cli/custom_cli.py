...
def handle_custom_command_management(
    args: str, console: Console
) -> CommandResult:
...
def _handle_custom_list(console: Console) -> CommandResult:
    ...
    for name, cmd in sorted(commands.items()):
        ...
        if cmd.command_type == "shell":
            ...
            if dry_run:
                console.print(f"    [yellow][dim]Mode: dry-run[/dim][/yellow]")
...
def _handle_custom_edit(console: Console, args: str) -> CommandResult:
    ...
    if not config_path.exists():
        manager = get_custom_manager()  # This will create the example config
        if not config_path.exists():
    ...
    try:
        ...
        try:
            import questionary
            reload_needed = questionary.confirm(
                "Reload custom commands now?",
                default=True
            ).ask()
            if reload_needed:
                return _handle_custom_reload(console)
        ...
        console.print(f"[dim]Try: /custom edit nano or export EDITOR=nano[/dim]")
